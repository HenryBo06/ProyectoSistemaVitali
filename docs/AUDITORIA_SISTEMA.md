...
| `app.py` | Panel de administración, cartera y recomendación del vendedor, revisión y descarga a producción. | Acceso inicial protegido por código en despliegue de red; modo local vinculado a `127.0.0.1`. |
...
| Alta | El primer visitante de una instancia nueva podía crear el administrador. | Instancia de red exige `SMARTORDER_SETUP_CODE` privado de al menos 20 caracteres; modo local explícito. |
